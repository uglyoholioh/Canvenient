...
export default function NotesView({ token, initialNoteId = null }) {
  ...
  const add = useCallback(
    async (targetPane = activePane) => {
      const note = await createNote({ title: "Untitled", content: "" }, token);
      setNotes((current) => [note, ...current]);
      setOpenTabs((prev) => ({
        ...prev,
        [targetPane]: prev[targetPane].includes(note.id)
          ? prev[targetPane]
          : [...prev[targetPane], note.id],
      }));
  ...
  const handleDelete = useCallback(
    async (id) => {
      ...
      try {
        ...
        confirmed = await tauriConfirm("Are you sure you want to delete this note?", {
          title: "Canvenient",
          type: "warning",
        });
      } catch {
  ...
  const toolbarConfig = useMemo(
    () => ({
      ...
      subtitle: notes.length
        ? `${notes.length} ${notes.length === 1 ? "note" : "notes"}${isSplit ? " • Split view" : ""}`
        : "No notes yet",
      actions: null,
    }),
        ...
        >
          ...
          <div
            style={{
              overflow: "hidden",
              flex: 1,
              paddingRight: hoveredNoteId === note.id ? "52px" : "24px",
            }}
          >
            <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
              <span
                style={{
                  fontSize: "13px",
                  fontWeight: isSelectedInActive ? "600" : "400",
                  whiteSpace: "nowrap",
                  overflow: "hidden",
            ...
            return (
              ...
              >
                <FileText
                  ...
                  style={{
                    flexShrink: 0,
                    color: noteObj?.is_pinned ? "var(--blue)" : "var(--text-muted)",
                  }}
                />
                <span
                  style={{
                ...
                <button
                  ...
                  aria-label={
                    isSplit
                      ? `Open tab in ${paneIndex === 0 ? "right" : "left"} pane`
                      : "Open tab alongside (split right)"
                  }
                  title={
                    isSplit
                      ? `Open tab in ${paneIndex === 0 ? "right" : "left"} pane`
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <h2 style={{ margin: 0, fontSize: "18px", fontWeight: "600", color: "var(--text-h)" }}>
            Notes
          </h2>
          <div
            style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <button
            ...
            style={{
              ...
              alignItems: "center",
              gap: "6px",
              background: isSplit ? "var(--surface-warm)" : undefined,
            }}
          >
            <Columns2 size={14} />
                            ...
                            {allTags.map((tag) => (
                              <button
                                ...
                                style={{
                                  ...
                                  border: "1px solid var(--border-strong)",
                                  background:
                                    selectedTag === tag ? "var(--blue)" : "var(--surface)",
                                  color: selectedTag === tag ? "#fff" : "var(--text)",
                                  cursor: "pointer",
                                }}
                              >
                                #{tag}
