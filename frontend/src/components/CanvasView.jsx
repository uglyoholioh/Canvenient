...
export default function CanvasView({ token }) {
  ...
  const toolbarConfig = useMemo(() => ({
    title: "Canvas",
    subtitle: selectedCourse ? selectedCourse.course_code : "All courses",
    actions: (
      <>
  ...
  }), [selectedCourse, syncing, sync, tab]);
  ...
  return (
    <div className="canvas-page">
      <aside className="canvas-course-sidebar">
        ...
        {courses.map((course) => <button type="button" key={course.id} style={{ "--module-color": course.color }} className={String(selectedCourseId) === String(course.id) ? "is-active" : ""} onClick={() => { setSelectedCourseId(course.id); setSelectedFile(null); }}><span className="course-dot" /> <span><strong>{course.course_code}</strong><small>{course.name}</small></span></button>)}
      </aside>
