...
function normalizedTypes(types = []) {
  const values = [];
  if (types.includes("online_text_entry") || types.includes("text_entry"))
    values.push("text_entry");
  ...
  const submit = async (event) => {
    ...
    try {
      if (type === "file_upload") {
        ...
        if (!content.trim())
          throw new Error(type === "online_url" ? "Enter a URL." : "Enter your submission text.");
      ...
      <form className="submit-modal" onSubmit={submit}>
        ...
        {types.length === 0 ? (
          <div className="module-empty">
            This assignment does not accept an online submission supported by Canvenient.
          </div>
        ) : (
          <>
            <div className="submit-tabs">
              {types.includes("text_entry") && (
                <button
                  type="button"
                  className={type === "text_entry" ? "is-active" : ""}
                  onClick={() => setType("text_entry")}
                >
                  <Type size={13} />
                  Text
                </button>
              ...
              {types.includes("online_url") && (
                <button
                  type="button"
                  className={type === "online_url" ? "is-active" : ""}
                  onClick={() => setType("online_url")}
                >
                  <Link size={13} />
                  URL
                </button>
              ...
              {types.includes("file_upload") && (
                <button
                  type="button"
                  className={type === "file_upload" ? "is-active" : ""}
