...
export default function CanvasDrawer({ item, token, onClose }) {
  ...
  useEffect(() => {
    const handleTasksChanged = () => { checkTaskStatus(); };
  ...
  const handleAddToTasks = async () => {
    ...
    try {
      ...
      const existing = (existingTasks || []).find((t) => t.source_type === "canvas" && t.source_id === sourceId);
      ...
      if (existing) {
        ...
        const module = (modules || []).find(
          (m) => String(m.source_course_id) === String(detail.course_id) || m.module_code === detail.course_code
        );
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {isAssignment && (
              <button
                ...
                title={isAddedToTasks ? "Already added to Tasks" : "Add this assignment as a task"}
              >
                {addingTask ? (
                  <>
