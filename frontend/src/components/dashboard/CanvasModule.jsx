...
function dueLabel(value) {
  ...
  if (dayDistance > 1 && dayDistance < 7)
    return `Due ${due.toLocaleDateString([], { weekday: "short" })} · ${time}`;
...
export default function CanvasModule({ token, enabled, onOpenItem, onNavigate }) {
  const [assignments, setAssignments] = useState(() =>
    getCachedData("canvenient.cache.assignments", []),
  );
  const [announcements, setAnnouncements] = useState(() =>
    getCachedData("canvenient.cache.announcements", []),
  );
  ...
  const upcoming = useMemo(
    () =>
      assignments
        .filter(
          (item) => !item.has_submitted && (!item.due_at || new Date(item.due_at) >= new Date()),
        )
        .sort(
          (left, right) =>
  ...
  const unreadAnnouncementsCount = useMemo(
    () => announcements.filter((item) => !item.is_dismissed).length,
    [announcements],
  );
  ...
  const addToTasks = async (assignment) => {
    ...
    try {
      ...
      const existing = (existingTasks || []).find(
        (task) => task.source_type === "canvas" && task.source_id === sourceId,
      );
      ...
      if (existing) {
        ...
        const module = (modules || []).find(
          (item) =>
            String(item.source_course_id) === String(assignment.course_id) ||
            item.module_code === assignment.course_code,
        );
  ...
  const hasCachedStore = (() => {
    try {
      return (
        localStorage.getItem("canvenient.cache.assignments") !== null ||
  ...
  const hasCachedContent =
    (assignments && assignments.length > 0) || (announcements && announcements.length > 0);
  ...
  if (!hasCachedContent && !hasCachedStore && refreshing) {
    return (
      <div className="module-empty">
        <Loader2 className="retro-icon-spin" size={14} /> Loading Canvas...
      </div>
    );
  }
    ...
    <div className="canvas-module">
      ...
      <section className="canvas-module-section">
        <div className="module-subheading">
          <span className="canvas-subheading-label">
          ...
          <div className="canvas-sync-status-group">
            ...
            >
              ...
              {unreadAnnouncementsCount > 0 && (
                <span className="canvas-triage-badge">{unreadAnnouncementsCount}</span>
              )}
            </button>
            {refreshing ? (
              <span
                className="canvas-syncing-badge"
                role="status"
                aria-label="Syncing with Canvas"
                title="Fetching latest Canvas data"
              >
        ...
        {upcoming.length === 0 ? (
          <div className="module-empty compact">No unsubmitted deadlines ahead.</div>
        ) : (
          <div className="canvas-compact-list">
            {upcoming.map((item) => (
              <div className="canvas-compact-row" key={`assignment-${item.course_id}-${item.id}`}>
                <button
                  type="button"
                  className="canvas-compact-row-main"
                  onClick={() => onOpenItem({ ...item, itemType: "assignment" })}
                >
                  <span className="canvas-course-code">{item.course_code}</span>
                ...
                <button
                  ...
                  aria-label={
                    isAssignmentAdded(item)
                      ? `${item.title} added to Tasks`
                      : `Add ${item.title} to Tasks`
                  }
                >
                  {isAssignmentAdded(item) ? (
                    <Check size={13} />
                  ) : addingId === item.id ? (
                    <Loader2 className="retro-icon-spin" size={13} />
                  ) : (
                    <Plus size={13} />
                  )}
                </button>
