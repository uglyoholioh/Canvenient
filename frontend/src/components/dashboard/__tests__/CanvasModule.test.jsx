...
describe("CanvasModule", () => {
    ...
    getCanvasAssignments.mockResolvedValue([
      {
        ...
        course_code: "ST2334",
        title: "Assignment 1",
        due_at: "2099-09-02T15:59:00Z",
        description: "<p>Please solve <b>problem 1</b> &amp; 2.</p>" + "A".repeat(5000),
        external_url: "https://canvas.test/assignment",
      },
      {
        id: 11,
        course_id: 1,
        course_code: "ST2334",
        title: "Quiz 1",
        due_at: "2099-09-04T15:59:00Z",
      },
    ]);
    getCanvasAnnouncements.mockResolvedValue([
      {
        id: 20,
        course_id: 1,
        course_code: "ST2334",
        ...
        is_priority: true,
        posted_at: "2099-09-01T00:00:00Z",
      },
    ]);
  ...
  it("opens the matching deadline details", async () => {
    ...
    expect(onOpenItem).toHaveBeenCalledWith(
      expect.objectContaining({ itemType: "assignment", id: 10 }),
    );
  });
  ...
  it("imports a Canvas deadline to Tasks with stripped HTML and character limit", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: "Assignment 1 added to Tasks" }),
    ).toBeDisabled();
  });
  ...
  it("reopens a completed canvas task when clicked", async () => {
    getTasks
      .mockResolvedValueOnce([
        { id: 40, source_type: "canvas", source_id: "canvas:1:10", status: "done" },
      ])
      .mockResolvedValueOnce([
        { id: 40, source_type: "canvas", source_id: "canvas:1:10", status: "done" },
      ])
  ...
  it("shows cached Canvas data immediately while refresh is pending", async () => {
    ...
    getCanvasAssignments.mockReturnValue(
      new Promise((resolve) => {
