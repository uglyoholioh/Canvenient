...
describe("TasksModule controls", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getTasks.mockResolvedValue([{ id: 7, title: "Read chapter", status: "todo", created_at: "2026-09-01T10:00:00Z" }]);
    updateTask.mockResolvedValue({ id: 7, title: "Read chapter two", status: "todo", created_at: "2026-09-01T10:00:00Z" });
    createTask.mockResolvedValue({ id: 9, title: "New assignment", status: "todo", created_at: "2026-09-01T10:05:00Z" });
  });
  ...
  it("opens and saves an editor when edit button is clicked, then restores row focus", async () => {
    ...
    await waitFor(() => expect(updateTask).toHaveBeenCalledWith("token", 7, expect.objectContaining({ title: "Read chapter two" })));
  ...
  it("opens external url on task click when external_url exists", async () => {
    ...
    getTasks.mockResolvedValue([{ id: 8, title: "Online quiz", external_url: "https://canvas.test/quiz", status: "todo", created_at: "2026-09-01T10:00:00Z" }]);
    render(<TasksModule token="token" />);
    const task = await screen.findByRole("button", { name: /^Online quiz/ });
    fireEvent.click(task);
    expect(windowOpenSpy).toHaveBeenCalledWith("https://canvas.test/quiz", "_blank", "noopener,noreferrer");
  ...
  it("supports roving filter tabs and cancels an editor with Escape", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("button", { name: /^Read chapter/ })).toHaveFocus());
  });
  ...
  it("starts next-line entry on click, creates task on Enter and stays active for rapid entry", async () => {
    ...
    await waitFor(() => expect(createTask).toHaveBeenCalledWith("token", {
