...
describe("Canvas link handling", () => {
  it("extracts unique HTTP links and resolves Canvas-relative file links", () => {
    expect(extractCanvasLinks('<a href="/courses/1/files/9/download">Week 1 slides</a><a href="https://example.com/read">Reading</a><a href="/courses/1/files/9/download">Duplicate</a><a href="javascript:alert(1)">Ignore</a>')).toEqual([
  ...
  it("only treats browser-safe module URLs as external resources", () => {
    expect(moduleItemExternalUrl({ external_url: "https://canvas.nus.edu.sg/courses/1/files/9" })).toBe("https://canvas.nus.edu.sg/courses/1/files/9");
