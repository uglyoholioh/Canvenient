...
async def create_invite(payload: InviteCreate, current_user: CurrentUser):
    ...
    if not member or member["role"] != "admin":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only group admins can create invite links.")
    ...
    row = await db.fetch_one(
        query="""
            ...
            RETURNING code, g_id
        """,
        values={"creator_id": current_user.id, "code": code, "g_id": payload.g_id},
    )
...
async def join_group(code: str, current_user: CurrentUser):
    ...
    return {"message": "Successfully joined group!", "g_id": g_id}
