...
- **Signing prepared, activation awaits the owner:** `rebuild-install-macos.sh` now auto-detects a "Developer ID Application" certificate, signs via `APPLE_SIGNING_IDENTITY`, notarizes with notarytool (keychain profile `canvenient-notary` or APPLE_ID/APPLE_PASSWORD/APPLE_TEAM_ID), staples, and Gatekeeper-assesses the staged app before install. With no certificate (current state — verified 0 identities) it behaves exactly as before and says so. The owner must enroll in Apple Developer and create the certificate to activate signing; the request was posed on 2026-09-11 and remains unanswered, so the item stays open-by-default rather than declined.
